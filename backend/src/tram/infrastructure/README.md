# Внешние адаптеры

SQL, файловые форматы, HTTP, конфигурация и контракт. Здесь переводятся внешние данные в документы приложения; предметная формула не должна зависеть от поставщика.

## Откуда поступают данные и куда уходят

| Вход | Файлы обработки | Выход |
|---|---|---|
| manifest + JSONL | `bundles.py`, `publication.py` | Проверяемые документы приложения и транзакционная запись ревизии. |
| Уже агрегированный CSV | `csv_import.py` | Новый prepared bundle; это не очистка произвольной исходной таблицы. |
| Point GeoJSON data.mos.ru | `geo_import.py` | Нормализованные геообъекты + ссылка/хеш в снимке. |
| Ответ погодного/событийного API | `sources.py`, `context_store.py` | Нормализованный снимок с доступностью либо состоянием ошибки. |
| Запросы сценариев и задания | `repository.py`, `trips.py`, `evaluation.py` | Постоянные ряды, очередь, журнал рейсов и отчёты в таблицах `database.py`. |

## Что реализовать для реальных источников

Адаптеры полученных 624/62743 реализованы в `metro_files.py`: проверяют оригиналы и provenance, дают типизированные входы сценарию подготовки, сохраняют результаты с хешами. [Команды и файлы](../../../../docs/METRO_PIPELINE.md). Это поддержка двух проверенных схем; общий GeoJSON importer не понимает автоматически произвольные портальные `Cells/geoData`, а CSV importer требует фиксированный набор колонок.

Для OSM нужен отдельный адаптер; для событий — полный обход страниц и версии анонсов; для исторической погоды — архивный загрузчик с проверкой временной доступности. Сырые HTTP-ответы текущим `sources.py` автоматически не сохраняются в `sources/`. Будущий загрузчик артефакта модели также относится к внешним адаптерам; формулы и orchestration остаются в своих слоях.

На ревью проверить ограничения размера/таймаутов, валидацию и единицы, секреты в ошибках/логах, схему времени и транзакционность. Исправления внешних данных дают новую ревизию, а не незаметную перезапись опубликованной.

## Файлы

| Файл | Назначение |
|---|---|
| [__init__.py](__init__.py) | Маркер Python-пакета; не открывает БД, сеть и не запускает процессы при импорте. |
| [bundles.py](bundles.py) | Чтение и структурная проверка manifest.json и последовательного observations.jsonl. |
| [context_store.py](context_store.py) | Запись и чтение неизменяемых снимков, фильтры provider/kind и пагинация. |
| [contract.py](contract.py) | Загрузка OpenAPI, JSON Schema, строгий JSON без дубликатов ключей и нечисловых NaN. |
| [csv_import.py](csv_import.py) | Потоковое преобразование точного CSV-формата агрегатов в prepared bundle. |
| [database.py](database.py) | SQLAlchemy-модели, UTC timestamp, engine/session factory и ограничения таблиц. |
| [demo.py](demo.py) | Синтетические данные текущей даты, сеть из четырёх остановок, запросы, план/журнал рейса и отсечки оценки. |
| [evaluation.py](evaluation.py) | Выборка фактов и транзакционная публикация проверенных по контракту отчёта и точек оценки. |
| [geo_import.py](geo_import.py) | Импорт WGS84 Point GeoJSON Москвы с явными полями, проверками координат/ID и SHA-256. |
| [metro_files.py](metro_files.py) | Разбор 624/62743 и provenance, проверка хешей/типов/координат, запись и чтение файловых исследовательских наборов. |
| [publication.py](publication.py) | SQL-публикация набора одной транзакцией, хеширование потока, пакетные вставки и блокировки повторов. |
| [repository.py](repository.py) | SQL-чтение версий/точек/отчётов, очередь, идемпотентность запросов, lease/fencing и публикация прогнозов. |
| [runtime.py](runtime.py) | Системные часы и подписанные HMAC курсоры пагинации. |
| [settings.py](settings.py) | Типизированные TRAM_* настройки из .env/окружения; SecretStr и проверка ключей. |
| [sources.py](sources.py) | Ограниченные HTTP-клиенты Open-Meteo, WeatherAPI, KudaGo, Timepad и нормализация ответов без утечки ключей. |
| [trips.py](trips.py) | Постоянное состояние рейсов и журнал checkpoints, PostgreSQL-блокировки по trip_id. |
