...
Поддерживаемые файлы проекта. Временные данные, секреты, зависимости и результаты сборки исключены из Git. Для sources/data/models отслеживаются только README. Назначение файлов и входы/выходы находятся в README соответствующих папок. **Единственный глобальный план и актуальная очередь работ:** [00_ГЛОБАЛЬНЫЙ_ПЛАН_К_ЭТАЛОНУ.md](../00_ГЛОБАЛЬНЫЙ_ПЛАН_К_ЭТАЛОНУ.md).

## .

| Файл | Назначение |
|---|---|
| [00_ГЛОБАЛЬНЫЙ_ПЛАН_К_ЭТАЛОНУ.md](../00_ГЛОБАЛЬНЫЙ_ПЛАН_К_ЭТАЛОНУ.md) | Главный план разработки данных, модели, обучения и backend; первая ссылка в корневом README. |
...
| [TRAINING_PLAN.md](../docs/TRAINING_PLAN.md) | Справочные заметки по квартальному metro experiment; актуальная очередь реализации находится в глобальном плане в корне репозитория. |
