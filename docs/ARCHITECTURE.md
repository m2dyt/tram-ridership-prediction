...
## Файловый эксперимент метро

`metro_commands.py` собирает отдельные `prepare-metro` и `evaluate-metro` до загрузки настроек БД. `infrastructure/metro_files.py` читает выгрузки 624/62743, проверяет provenance/хеши/типы и пишет неизменяемые каталоги. `domain/metro.py` определяет квартал, точку, разбиение и идентичность пары названий. `application/metro.py` чисто строит реестр, плотную сетку и отчёт качества: без SQL, файловых операций и ML-зависимостей.

`tram_ml/quarterly.py` принимает domain-типы и рассчитывает последовательные квартальные прогнозы/метрики без I/O и обучения. CLI сохраняет результаты через файловый адаптер. `manifest.json` служит маркером завершения и содержит хеши; каталоги не перезаписываются. API, worker и их сетка времени не расширялись: это отдельный исследовательский формат.

ID обозначает точную пару станция/линия, а физические объекты и переименования требуют проверенного соответствия. Пропуски явные, нули сохраняются, историческая доступность текущего релиза не выдумывается. [Схема файлов, временной протокол и ограничения](METRO_PIPELINE.md).
